...
charset = '<http-equiv="X-UA-Compatible" content="IE=edge";charset=utf-8" />\n'

tags = ['html', 'body', 'head', 'link',  'div', 'p', 'form', 'legend', 
...
for t in selfClose: setattr(thisModule, t, TagFactory(t))
def ValidW3C():
...
class PyH(Tag):
    ...
    def __init__(self, name='MyPyHPage'):
        self += head()
        self += body()
        self.attributes = dict(lang='zh-CN')
